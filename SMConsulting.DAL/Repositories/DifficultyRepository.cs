...
namespace SMConsulting.DAL.Repositories
{
    public class DifficultyRepository :GenericRepository<Difficulty>, IDifficultyRepository
    {
        ...
        public DifficultyRepository(SmDbContext context) : base(context)
        {
            _context = context;
        }

        public async Task<bool> IsExistAsync(int id)
      => await Table.AnyAsync(x => x.DifficultyId == id);

        public async Task<bool> RemoveAsync(int id)
        {
            int result = await Table.Where(x => x.DifficultyId == id).ExecuteDeleteAsync();
            return result > 0;
        }
    }
}
